...
exports.clean = function(str, is_image, whitelistNaughtyTags) {
    ...
    do {
        ...
        if (str.match(/<a/i)) {
            str = str.replace(/<a\s+([^>]*?)(>|$)/gi, function(m, attributes, end_tag) {
                ...
                filtered_attributes = filtered_attributes.replace(/\s+(href="(?:.*)?(?:alert\(|alert&#40;|javascript:|livescript:|mocha:|charset=|window\.|document\.|\.cookie|<script|<xss|data\s*:)(?:[^"]*))/gi, ' ');
                filtered_attributes = filtered_attributes.replace(/\s+(href='(?:.*)?(?:alert\(|alert&#40;|javascript:|livescript:|mocha:|charset=|window\.|document\.|\.cookie|<script|<xss|data\s*:)(?:[^'']*))/gi, ' ');
...
//(?!  (?:"|')   (?:\s|\/|>)  )*
        if (str.match(/<img/i)) {
            str = str.replace(/<img\s+([^>]*?)(\s?\/?>|$)/gi, function(m, attributes, end_tag) {
                var filtered_attributes = filter_attributes(attributes.replace('<','').replace('>',''));
                filtered_attributes = filtered_attributes.replace(/\s+(src="(?:.*)?(?:alert\(|alert&#40;|javascript:|livescript:|mocha:|charset=|window\.|document\.|\.cookie|<script|<xss|base64\s*,)(?:[^"]*))/gi, ' ');
                filtered_attributes = filtered_attributes.replace(/\s+(src='(?:.*)?(?:alert\(|alert&#40;|javascript:|livescript:|mocha:|charset=|window\.|document\.|\.cookie|<script|<xss|base64\s*,)(?:[^'']*))/gi, ' ');
                return m.replace(attributes, filtered_attributes);
            });
        }
    ...
    //So this: <blink>
    //Becomes: &lt;blink&gt;
    var naughty = 'alert|applet|audio|basefont|base|behavior|bgsound|blink|body|embed|expression|form|frameset|frame|head|html|ilayer|iframe|input|isindex|layer|link|meta|object|plaintext|style|script|textarea|title|video|xml|xss'.split('|');

    // allow whitelisting specific tags if necessary
    var blacklist = [];
    for (var i = 0; i < naughty.length; i++) {
      if ( (whitelistNaughtyTags || []).indexOf(naughty[i] === -1)) {
        blacklist.push(naughty[i]);
      }
    }

    str = str.replace(new RegExp('<(/*\\s*)(' + blacklist.join('|') + ')([^><]*)([><]*)', 'gi'), function(m, a, b, c, d) {
